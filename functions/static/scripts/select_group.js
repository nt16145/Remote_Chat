...
function getGrouplist() {
  firebase.auth().onAuthStateChanged(function (user) {
    if (user) {
      firestore.collection('groups').where('groupOwner', '==', user.uid).get()
      ...
      firestore.collection('groups').where('groupMember', '==', user.uid).get()
        .then(function (querySnapshot) {
...
document.addEventListener('click', function (e) {
  ...
  if (t.nodeName == "INPUT" && t.type == "button") {
    var a = Array.prototype.map.call(t.parentNode.parentNode.querySelectorAll('td'), function (n) {
